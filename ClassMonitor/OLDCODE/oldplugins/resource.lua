...
Engine.CreateResourceMonitor = function(name, enable, text, autohide, hideifmax, anchor, width, height, colors, specs)
	local cmResource = CreateFrame("Frame", name, UI.PetBattleHider)
...
--print("COMBAT:"..tostring(inCombat))
		if autohide == true and not inCombat then
			self:Hide()
			return
		end
		...
		if hideifmax == true and valueMax == valueCurrent and not inCombat then
			self:Hide()
			return
		end
